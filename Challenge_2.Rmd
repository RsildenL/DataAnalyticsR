...
```{r}
### take a second look at this (KARMAN) KARMAN-Looks Good :)
...
```{r}
range(train$delta) # should it not be between 0 and 1 since we normalized?
plot(failure~delta, data=train, col="red4")
lines(train$delta, m_predictions, col="green4", lwd=2) #Option1 - lengths differ

library(ggplot2) # Option2 - same
ggplot(train, aes(x=m_predictions , y=train$failure)) +
  geom_point()  +  stat_smooth(method="glm", se=FALSE, method.args = list(family=binomial)) 
```
