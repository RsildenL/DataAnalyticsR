...
message('AUC of model on the training set: ', auc_train)
message('AUC of model on the test set: ', auc_test)
```
...
message('delta_med: ', delta_med)
message('delta_high: ',delta_high)
```
...
message('Average annual percentage increase in revenue for med-risk EWS: ', annual_perc_incr(dt_alarm_med, delta_med))

## ii. And if we set the EWS only with the high-risk alarms?
message('Average annual percentage increase in revenue for high-risk EWS: ', annual_perc_incr(dt_alarm_high, delta_high))
```
