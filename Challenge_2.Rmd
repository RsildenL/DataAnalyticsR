...
dt_alarm_med = merged_data[delta > delta_med] # Assumption: Filter both medium and high risk because high risk instances imply having a medium risk of failure as well.

conversion_factor <- 24/dt_alarm_med$daily_sales_machine

dt_alarm_med[, threshold_hours:=delta_med*conversion_factor]

dt_alarm_med[, threshold_hours_fixed := threshold_hours + 1.5] # The 1.5 stands for the average time it takes to fix a machine.

dt_alarm_med[, delta_fixed:= threshold_hours_fixed * (1/(24/daily_sales_machine))]

dt_alarm_med[, won_sales := failure*(delta-delta_fixed)]

add_rev = sum(dt_alarm_med$won_sales*1.7) # 

# Cost of the system
sys_cost = 10*(nrow(dt_alarm_med[failure == 0]))

# Cost of current system per year
...
# As we are only given data from Jan 1st to April 1st we have to multiply our estimated profit increase by 4 to obtain the actual annual profit increase.
annual_profit_incr = 4*(add_rev - vendex_cost_prev) - sys_cost

# Again, we have to multiply the denominator by 4 so that we receive the annual profit increase in percentage.
perc_incr = annual_profit_incr/(nrow(merged_data)*1.7*4)
```
