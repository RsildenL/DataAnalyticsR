...
#machine_daily_average <- merged_data[, c('machine', 'date', 'avg_daily_sales')]
#machine_daily_average <- machine_daily_average[, mean_daily_sales := mean(.N), by = c('machine', 'date')]
...
merged_data$delta <- merged_data[ ,.(delta = as.numeric(deltahours/(24/avg_daily_sales))),] #changed delta to numeric
head(merged_data)    
```
...
```
** Highly imbalanced dataset (keeping similar distribution in test and train set)
...
print('training set:'); auc_train
print('test set:'); auc_test
...
print('Avg. number of medium-risk alarms per day:')
average_per_day_med
print('Avg. number of high-risk alarms per day:')
average_per_day_high
```
...
print('Percentage of medium-risk alarms that are false alarms:')
round(med_false_alarm, 2)
print('Percentage of high-risk alarms that are false alarms:')
round(high_false_alarm, 2)
```
...
print('Average annual percentage increase in revenue for med-risk EWS:')
annual_perc_incr(dt_alarm_med, delta_med)

## ii. And if we set the EWS only with the high-risk alarms?
print('Average annual percentage increase in revenue for high-risk EWS:')
