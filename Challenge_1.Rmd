...
<<<<<<< HEAD
<<<<<<< HEAD
There are 2495 vending machines, of which 38% (959) are small and 62% (1536) are large vending machines.
More than half of the vending machines are located close to transportation, while only about 14% are located close to a petrol station. 
=======
...
>>>>>>> bcb5a1f6b78267dc73939e223b75a13f7f0e4b39
=======
...
>>>>>>> 4a7e3e511a42e3a09af438496815a502f46c4378
...
<<<<<<< HEAD
'Carbonates and energy drinks' is the category with the most (13) products, followed by 'Chocolate based' snacks with 11 different products. 
The 'Milk based' drinks have the highest average price of 3.43, while 'Sugar candy' snacks have the lowest average price of 2.30.
=======
...
<<<<<<< HEAD
<<<<<<< HEAD
* drinks: a linear increasing trend (ANOVA?)
=======
* drinks: an linear increasing trend (ANOVA?) - increasing with temprature?
>>>>>>> 5f51a51f0e9d04db7218619fd1298e86dad9923d
* snack: flat/no trend
=======
...
The reason for the growth in drinks sold between January and April could be related to the external factor of the weather. While people presumably tend to buy warm drinks when the temperature is comparably low outside, the vending machines cannot provide the customers with the warm drinks they want. As the temperature rises over time, people probably tend to buy more cold drinks again, which could explain the rising trend in drinks sold by the vending machines from January to April. For snacks on the other hand, the influence of this external factor does not apply, as the consumption of snacks do not rely as heavily on the weather as drinks do.
...
#3: substitute missing values with mean of small machine and categories 
```
=======
```{r}
income_NA <- machine_data[is.na(income_average), ]
...
transactions = transactions[, .N, list(date, machine)]  
data = merge(machines, transactions, all=TRUE)
data[, train_AvgDailyPassengers := ifelse(is.na(train_AvgDailyPassengers),1,0), ]
data[, income_average:= ifelse(is.na(income_average),0,income_average)]
data[, total_number_of_routes_600:= ifelse(is.na(total_number_of_routes_600),0,total_number_of_routes_600)]
m0 = glm(N ~ small_machine + income_average + total_number_of_routes_600 + num_hotels_45 + train_AvgDailyPassengers + num_vendex_nearby_300, data= data)
...
data[, log_transport:= log(total_number_of_routes_600, base= 10)]
data = data[!is.infinite(log_transport)]
m1 = glm(N ~ small_machine + income_average + log_transport + num_hotels_45 + train_AvgDailyPassengers + num_vendex_nearby_300, data= data)
summary(m1)
=======
