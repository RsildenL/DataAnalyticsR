...
To deal with missing values we can just to delete rows when a missing value appears. Our problem is fixed, however this result leads to having less data could affect the distribution our data, hence the predict capabilities of our model. The advantage of this method is that there are no approximation that could lead to noice in the model
...
Therefore, another way to deal with missing values is to replace the missing values with the mean variable, more reaonable value than zero, for the income average. The advantage is that we keep those instances but this could create some correlation problems and affect the variability of our data.
```{r}
...
For this specific case we choose to replace the missing value with the mean income average of machine type and location type of the the instance where we have a missing value.This is an improvement from  the overall average in the last option, as this uses the mean of machines with more similar characteristics
