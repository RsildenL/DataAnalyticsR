...
```
There are 2395 vending machines, wheras 38% (959) of them are small and 62% (1536) are large vending machines.
...
plot(daily_items_sold$date, daily_items_sold$N)
```
...
```{r}
summary(machine_data$income_average)  ### Why do we not get the same results ?? 
...
```{r}
#m0 <- glm( ~ small_machine + income_average + total_number_of_routes_600
...
```{r}
#m1 <- glm( ~ small_machine + income_average + log_transport
#     + num_hotels_45 + petrol_station + num_vendex_nearby_300, 
#    data = merged_data)
```
