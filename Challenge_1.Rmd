...
sales_dist2 <- sales_dist[,.(num_machines= .N, tot_products = sum(N), avg_sales = round(mean(mean_sales),2)), by = small_machine]
...
mean(drink_trend[day=="Sunday"|day=="Saturday"]$avg_daily) # weekend average of 4.57
mean(drink_trend[!(day=="Sunday"|day=="Saturday"),]$avg_daily) # week average of 3.39
perday<-drink_trend[,day_avg:=mean(avg_daily)]
week_trend<-drink_trend[,.(day_avg=mean(avg_daily)),by=day]
plot(1:7,week_trend$day_avg,type = "l")
```

```{r}
snack_trend[,day:=weekdays(date)]
mean(snack_trend[day=="Sunday"|day=="Saturday"]$avg_daily) # weekend average of 7.25
mean(snack_trend[!(day=="Sunday"|day=="Saturday"),]$avg_daily) # week average of 5.61
perday <- snack_trend[,day_avg:=mean(avg_daily)]
week_trend <- snack_trend[,.(day_avg=mean(avg_daily)),by=day]
plot(1:7,week_trend$day_avg,type = "l")
```
