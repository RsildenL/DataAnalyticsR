...
From the distribution of the daily sales it is clear that some machines have significantly more sales than others. This could indicate that some machines are more strategically placed than others. Another reason could be that the assortment is better.
...
```
Moving forward, the income average are chosen to be kept as is, with no adjustment of outliers. This is done, as we interpret the outliers not as unlikely observations, though are we aware of consequences that this can cause more noicy predictions in the models in question 5.
...
To deal with missing values we can just delete rows when a missing value appears. Our problem is fixed, however, this can affect the distribution of our data, hence the predictive capabilities of our model. The advantage of this method is that no approximation could lead to noise in the model.
...
Another approach could be to replace all missing values with zero, this is one way to avoid missing instances, but replacing with zero seems unreasonable as the minimum income of the income average is 31570. 
```{r}
...
A similar, but more reasonable approach to deal with missing values is to replace the missing values with the mean of the income average. The advantage is that we keep those instances but this could create some correlation problems and affect the variability of our data.
```{r}
...
For this specific situation, we choose to replace the missing value with the mean income average of the machine type and location type of the instance where we have a missing value. This is an improvement from  the overall average in the last option, as this uses the mean of machines with more similar characteristics
...
After removing statistically non-significant variables from our model and assuming all other factors remaining equal, a machine that is small sells on average 1.608906 fewer units per day than big machines.
```{r }
...
With other machines nearby and assuming all other factors remain the same, machine sales decrease by an average of 0.1055621 per day.
```{r }
...
```
Given the two options for locations taking into account the factors involved in each, we would choose the first option because we predict higher sales for this location (10.21088 instead of 6.191234 in daily sales variable), assuming that both locations cost us the same to acquire and maintain.
